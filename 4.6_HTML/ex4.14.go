...
import (
	"fmt"
	"github.com/pingjing0628/golang-practice/4.6_HTML/github"
	"html/template"
	"log"
	"net/http"
	"os"
...
var issueTemplate = template.Must(template.New("issues").Parse(navigation + `
...
var userTemplate = template.Must(template.New("users").Parse(navigation + `
...
func main()  {
	if len(os.Args) != 3 {
		fmt.Fprintln(os.Stderr, "usage: ex14 OWNER Repository")
		os.Exit(1)
	}
	...
	fmt.Println("Listening at http://localhost:9000")
	log.Fatal(http.ListenAndServe("localhost:9000", nil))
}

func handleIssues(w http.ResponseWriter, r *http.Request)  {
	issueTemplate.Execute(w, issues)
}

func handleUsers(w http.ResponseWriter, r *http.Request)  {
	issueTemplate.Execute(w, users)
}

func handleMilestones(w http.ResponseWriter, r *http.Request)  {
	issueTemplate.Execute(w, milestones)
}
